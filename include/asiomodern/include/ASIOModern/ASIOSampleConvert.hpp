...
template <typename Type> struct TraitsExternal {};

template <> struct TraitsExternal<std::int8_t> { using type = std::int8_t; using internal_type = std::int64_t; static constexpr internal_type scale = static_cast<internal_type>(1)<<24; };
template <> struct TraitsExternal<std::int16_t> { using type = std::int16_t; using internal_type = std::int64_t; static constexpr internal_type scale = static_cast<internal_type>(1)<<16; };
template <> struct TraitsExternal<std::int32_t> { using type = std::int32_t; using internal_type = std::int64_t; static constexpr internal_type scale = static_cast<internal_type>(1)<<0; };
template <> struct TraitsExternal<float> { using type = float; using internal_type = float; };
template <> struct TraitsExternal<double> { using type = double; using internal_type = double; };
template <> struct TraitsExternal<long double> { using type = long double; using internal_type = double; };
...
template <> constexpr double clip<double>(double dbl) noexcept {
	return std::clamp(dbl, -1.0, 1.0);
}
template <> constexpr float clip<float>(float flt) noexcept {
	return std::clamp(flt, -1.0f, 1.0f);
}



template <typename Tdst, typename Tsrc> inline Tdst convert_internal(Tsrc x) noexcept = delete;
template <> inline std::int64_t convert_internal<std::int64_t, std::int64_t>(std::int64_t q32) noexcept {
	return q32;
}
template <> inline std::int64_t convert_internal<std::int64_t, double>(double dbl) noexcept {
	return std::llround(dbl * static_cast<double>(1ll << 32));
}
template <> inline std::int64_t convert_internal<std::int64_t, float>(float flt) noexcept {
	return std::llround(flt * static_cast<float>(1ll << 32));
}
template <> inline double convert_internal<double, std::int64_t>(std::int64_t q32) noexcept {
	return static_cast<double>(q32) * (1.0/static_cast<double>(1ll << 32));
}
template <> inline double convert_internal<double, double>(double dbl) noexcept {
	return dbl;
}
template <> inline double convert_internal<double, float>(float flt) noexcept {
	return static_cast<double>(flt);
}
template <> inline float convert_internal<float, std::int64_t>(std::int64_t q32) noexcept {
	return static_cast<float>(q32) * (1.0f/static_cast<float>(1ll << 32));
}
template <> inline float convert_internal<float, double>(double dbl) noexcept {
	return static_cast<float>(dbl);
}
template <> inline float convert_internal<float, float>(float flt) noexcept {
	return flt;
}



template <typename SampleType> inline SampleType sample_from_value(typename Types<SampleType>::value_type val) noexcept {
...
template <typename SampleType> inline typename Types<SampleType>::value_type value_from_sample(SampleType smp) noexcept {
	if constexpr (Types<SampleType>::is_float) {
		static_assert(sizeof(SampleType) == sizeof(typename Types<SampleType>::value_type));
		static_assert((stdcxx20::endian::native == stdcxx20::endian::little) || (stdcxx20::endian::native == stdcxx20::endian::big));
		if constexpr (Types<SampleType>::is_be) {
...
template <typename SampleType> inline typename Types<SampleType>::value_type value_from_internal(typename Types<SampleType>::internal_type smp_int) noexcept {
	if constexpr (Types<SampleType>::is_float) {
...
template <typename SampleType> inline typename Types<SampleType>::internal_type internal_from_value(typename Types<SampleType>::value_type val) noexcept {
	if constexpr (Types<SampleType>::is_float) {
...
template <typename ExternalType> inline typename TraitsExternal<ExternalType>::internal_type internal_from_external(ExternalType x) noexcept {
...
template <typename ExternalType> inline ExternalType external_from_internal(typename TraitsExternal<ExternalType>::internal_type x) noexcept {
	static_assert(std::is_floating_point<typename TraitsExternal<ExternalType>::internal_type>::value == std::is_floating_point<ExternalType>::value);
	if constexpr (std::is_floating_point<ExternalType>::value) {
...
template <typename SampleType, typename ExternalType> inline SampleType sample_from_external(ExternalType x) noexcept {
...
template <typename ExternalType, typename SampleType> inline ExternalType external_from_sample(SampleType x) noexcept {
...
struct Traits {
	...
	{
		switch (type) {
		case SampleType::Int16MSB: { using Type = detail::Types<detail::Int16MSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int24MSB: { using Type = detail::Types<detail::Int24MSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32MSB: { using Type = detail::Types<detail::Int32MSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Float32MSB: { using Type = detail::Types<detail::Float32MSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Float64MSB: { using Type = detail::Types<detail::Float64MSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32MSB16: { using Type = detail::Types<detail::Int32MSB16>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32MSB18: { using Type = detail::Types<detail::Int32MSB18>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32MSB20: { using Type = detail::Types<detail::Int32MSB20>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32MSB24: { using Type = detail::Types<detail::Int32MSB24>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int16LSB: { using Type = detail::Types<detail::Int16LSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int24LSB: { using Type = detail::Types<detail::Int24LSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32LSB: { using Type = detail::Types<detail::Int32LSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Float32LSB: { using Type = detail::Types<detail::Float32LSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Float64LSB: { using Type = detail::Types<detail::Float64LSB>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32LSB16: { using Type = detail::Types<detail::Int32LSB16>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32LSB18: { using Type = detail::Types<detail::Int32LSB18>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32LSB20: { using Type = detail::Types<detail::Int32LSB20>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		case SampleType::Int32LSB24: { using Type = detail::Types<detail::Int32LSB24>; size_bytes = sizeof(Type::type); valid_bits = Type::valid_bits; is_float = Type::is_float; is_be = Type::is_be; } break;
		default: break;
		}
	}
};
