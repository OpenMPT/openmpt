...
#if !((_MSC_VER >= 1937) && (_MSC_VER < 1939))
...
static MPT_NOINLINE void TestSampleConversion()
{
	...
	{
		std::vector<mpt::byte> source8(256);
		for(std::size_t i = 0; i < 256; i++)
		{
			source8[i] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i));
		}
	...
	// Delta 16-Bit Integer PCM
	{
		...
		std::vector<mpt::byte> source16(65536 * 2);
		for(std::size_t i = 0; i < 65536; i++)
		{
			source16[i * 2 + 0] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i & 0xFF));
			source16[i * 2 + 1] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i >> 8));
		}
		...
		for(std::size_t i = 0; i < 65536; i++)
		{
			source16[i * 2 + 0] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i >> 8));
			source16[i * 2 + 1] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i & 0xFF));
		}
	...
	{
		std::vector<mpt::byte> source24(65536 * 3);
		for(std::size_t i = 0; i < 65536; i++)
		{
			source24[i * 3 + 0] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(0));
			source24[i * 3 + 1] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i & 0xFF));
			source24[i * 3 + 2] = mpt::byte_cast<mpt::byte>(static_cast<uint8>(i >> 8));
		}
	...
	// Float 32-Bit
	{
		std::vector<mpt::byte> source32(65536 * 4);
		for(std::size_t i = 0; i < 65536; i++)
		{
			IEEE754binary32BE floatbits = IEEE754binary32BE((static_cast<float>(i) / 65536.0f) - 0.5f);
			source32[i * 4 + 0] = mpt::byte_cast<mpt::byte>(floatbits.GetByte(0));
			source32[i * 4 + 1] = mpt::byte_cast<mpt::byte>(floatbits.GetByte(1));
			source32[i * 4 + 2] = mpt::byte_cast<mpt::byte>(floatbits.GetByte(2));
			source32[i * 4 + 3] = mpt::byte_cast<mpt::byte>(floatbits.GetByte(3));
		}
	...
	// Range checks
	{
		mpt::byte oneSample = mpt::byte_cast<mpt::byte>(static_cast<int8>(1));
		int8 targetBuf4[4];
