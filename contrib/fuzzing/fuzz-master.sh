...
sudo mount -t tmpfs -o size=150M none $FUZZING_TEMPDIR
rm -rf $FUZZING_TEMPDIR/bin
mkdir $FUZZING_TEMPDIR/bin
cp ../../bin/* $FUZZING_TEMPDIR/bin/
rm $FUZZING_TEMPDIR/bin/libopenmpt.so
cp ../../bin/libopenmpt.so.0 $FUZZING_TEMPDIR/bin/libopenmpt.so
cp ../../bin/libopenmpt.so.0 $FUZZING_TEMPDIR/bin/libopenmpt.so.0
