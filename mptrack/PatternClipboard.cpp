	...
	while(curRow < sndFile.Patterns[pattern].GetNumRows() || multiPaste)
	{
		...
		while((pos + 11 < data.GetLength()) && (data[pos] == '|'))
		{
			...
			if(skipPaste == false)
			{
				...
				if(m.IsPcNote())
				{
					m.SetValueEffectCol(std::min<uint16>(m.GetValueEffectCol(), ModCommand::maxColumnValue));
					m.SetValueVolCol(std::min<uint16>(m.GetValueVolCol(), ModCommand::maxColumnValue));
				}
