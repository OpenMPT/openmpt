...
//----------------------------------------------------------
{
	...
	m_MixerSettings.m_SampleFormat = CMainFrame::GetPrivateProfileLong("Sound Settings", "BitsPerSample", m_MixerSettings.m_SampleFormat, iniFile);
...
//------------------------------------------
{
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER,	m_csRegKey, 0, KEY_READ, &key) == ERROR_SUCCESS)
	{
		...
		DWORD dummy_sampleformat = m_MixerSettings.m_SampleFormat;
		RegQueryValueEx(key, "BitsPerSample", NULL, &dwREG_DWORD, (LPBYTE)&dummy_sampleformat, &dwDWORDSize);
		m_MixerSettings.m_SampleFormat = (long)dummy_sampleformat;
...
//----------------------------------
{
	...
	CMainFrame::WritePrivateProfileLong("Sound Settings", "BitsPerSample", m_MixerSettings.m_SampleFormat, iniFile);
