...
CInputHandler::KeyboardEvent CInputHandler::Translate(const MSG &msg)
{
	...
	if(key == VK_PACKET)
	{
		// This message was sent by something other than a physical keyboard.  This behaviour can be observed
		// when using Microsoft's RDP client for iOS or when using the Windows on-screen keyboard.
