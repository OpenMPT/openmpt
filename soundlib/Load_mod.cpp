...
bool CSoundFile::ReadMOD(FileReader &file, ModLoadingFlags loadFlags)
{
	...
	if(loadFlags & loadSampleData)
	{
		...
		else if(isMdKd && file.ReadArray<char, 6>() == std::array<char, 9>{0x00, 0x11, 0x55, 0x33, 0x22, 0x11} && file.CanRead(3))  // 3 more bytes that differ between modules and Tetramed version, purpose unknown
