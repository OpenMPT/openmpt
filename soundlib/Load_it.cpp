		...
		if(smpPos[i] > 0 && file.Seek(smpPos[i]) && file.ReadStruct(sampleHeader))
		{
			...
			} else
			{
				...
				if((loadFlags & loadSampleData) && strLen)
				{
					...
					AddToLog(LogWarning, mpt::format(MPT_USTRING("Loading external sample %1 ('%2') failed: External samples are not supported."))(i + 1, mpt::ToUnicode(mpt::CharsetUTF8, filenameU8)));
#endif // MPT_EXTERNAL_SAMPLES
				} else
				{
					file.Skip(strLen);
...
void CSoundFile::ReadMixPluginChunk(FileReader &file, SNDMIXPLUGIN &plugin)
{
	...
	pluginDataChunk.ReadRaw(plugin.pluginData.data(), plugin.pluginData.size());
	...
	if(modularData.IsValid())
	{
		while(modularData.CanRead(5))
		{
			...
			{
				plugin.fDryRatio = mpt::clamp(dataChunk.ReadFloatLE(), 0.0f, 1.0f);
