...
// -! CODE#0013
	...
	*/
	...
	//1. Writing tune specific tunings.
	if(m_TuningsTuneSpecific.SerializeBinary(fout))
	{
		errorMessages.Add("Saving tunespecific tunings failed");
		fout.setstate(ios::failbit);
	}

	//Instrument specifics - writing only if instruments exists
	if(GetNumInstruments() > 0)
	{
		...
		for(UINT i = 1; i<=GetNumInstruments(); i++)
		{
			...
			TNTS_MAP_ITER iter = tNameToShort_Map.find(Headers[i]->pTuning);
			if(iter != tNameToShort_Map.end())
				continue; //Tuning already mapped.
			
			tNameToShort_Map[Headers[i]->pTuning] = figMap;
			figMap++;
		}
		...
		if(tuningMapSize == 0)
		{
			errorMessages.Add("Tuning name to figure mapping failed");
		}
		else
		{	
			...
			//4. Writing tuning data for instruments.
			for(UINT i = 1; i<=GetNumInstruments(); i++)
			{
				TNTS_MAP_ITER iter = tNameToShort_Map.find(Headers[i]->pTuning);
				...
				{
					ASSERT(false);
					string str = string("Tuning of instrument ") + Stringify(i) + string(" might be lost.");
					errorMessages.Add(str, SM_NOTFATAL);
					iter = tNameToShort_Map.begin();
				}
...
// -> BEHAVIOUR_CHANGE#0008
	...
	if(version > 0x889)
	{
		...
		//use istream, instead of simply adding method 
		//to read from char pointer, I wanted to find out 
		//could it be done without adding such method - 
		//after battling with this for a _long_ time, some kind
		//of solution seemed to arise. So here creating
		...
		for(UINT i = 1; i<=GetNumInstruments(); i++)
		{
			...
			if(Headers[i] && iter != shortToTNameMap.end())
			{
				...
				Headers[i]->pTuning = s_TuningsSharedLocal.GetTuning(str);
				if(Headers[i]->pTuning)
					continue;

				Headers[i]->pTuning = m_TuningsTuneSpecific.GetTuning(str);
				if(Headers[i]->pTuning)
					continue;
				...
				if(iter == notFoundTunings.end())
				{
					...
					CMainFrame* p = CMainFrame::GetMainFrame();
			...
			{
				if(Headers[i])
					Headers[i]->pTuning = INSTRUMENTHEADER::s_DefaultTuning;
				ASSERT(false);
			}
