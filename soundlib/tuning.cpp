...
{
	mpt::IO::WriteTextCRLF(f, MPT_FORMAT("! %1")(mpt::ToCharset(mpt::Charset::ISO8859_1, (filename.GetFileName() + filename.GetFileExt()).ToUnicode())));
	...
	if(GetType() == Type::GEOMETRIC)
	{
		mpt::IO::WriteTextCRLF(f, MPT_FORMAT(" %1")(m_GroupSize));
		mpt::IO::WriteTextCRLF(f, "!");
		for(NOTEINDEXTYPE n = 0; n < m_GroupSize; ++n)
		{
			...
			mpt::IO::WriteTextCRLF(f, MPT_FORMAT(" %1 ! %2")(
				mpt::fmt::fix(cents),
	...
	{
		mpt::IO::WriteTextCRLF(f, MPT_FORMAT(" %1")(m_RatioTable.size() + 1));
		mpt::IO::WriteTextCRLF(f, "!");
