	...
	if(adjustMode & eAdjust)
	{
		if(retval.targetReached || target.mode == GetLengthTarget::NoTarget)
		{
			...
			for(CHANNELINDEX n = 0; n < GetNumChannels(); n++)
			{
				...
				m_PlayState.Chn[n].pCurrentSample = nullptr;
			}
...
void CSoundFile::RetrigNote(CHANNELINDEX nChn, int param, int offset)
{
	...
	if(doRetrig)
	{
		...
		offset--;
		if(chn.pModSample != nullptr && offset >= 0 && offset <= static_cast<int>(CountOf(chn.pModSample->cues)) && chn.pModSample != nullptr)
		{
	...
	if (pChn->dwFlags[CHN_SUSTAINLOOP] && pChn->pModSample && bKeyOn)
	{
		...
		if(pSmp->uFlags[CHN_LOOP])
		{
			...
			if(pChn->position.GetUInt() > pChn->nLength)
			{
				...
				pChn->position.Set(pChn->nLoopStart + ((pChn->position.GetInt() - pChn->nLoopStart) % (pChn->nLoopEnd - pChn->nLoopStart)));
			}
		} else
		{
