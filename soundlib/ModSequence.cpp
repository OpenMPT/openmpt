...
bool ModSequenceSet::MergeSequences()
{
	...
	for(SEQUENCEINDEX seqNum = 1; seqNum < GetNumSequences(); seqNum++)
	{
		...
		{
			m_sndFile.AddToLog(LogWarning, MPT_UFORMAT("WARNING: Cannot merge Sequence %1 (too long!)")(seqNum + 1));
			continue;
		}
				...
				if(m->command == CMD_POSITIONJUMP)
				{
					if(patternsFixed[pat] != SEQUENCEINDEX_INVALID && patternsFixed[pat] != seqNum)
					{
						...
						} else
						{
							...
							m_sndFile.AddToLog(LogWarning, MPT_UFORMAT("CONFLICT: Pattern break commands in Pattern %1 might be broken since it has been used in several sequences!")(pat));
						}
					}
...
void ReadModSequenceOld(std::istream& iStrm, ModSequenceSet& seq, const size_t)
{
	...
	{
		seq.m_sndFile.AddToLog(LogWarning, MPT_UFORMAT("Module has sequence of length %1; it will be truncated to maximum supported length, %2.")(size, ModSpecs::mptm.ordersMax));
