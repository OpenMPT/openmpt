...
build\tools\7zipold\7za.exe x -y -obuild\tools\7zipa "build\externals\7z2501-extra.7z" || goto error
build\tools\7zipa\7za.exe x -y -obuild\tools\7zip "build\externals\7z2501-x64.exe" || goto error
...
call build\scriptlib\unpack.cmd "build\tools\python3" "build\externals\python-3.14.2-embed-amd64.zip" "." || goto error


call :killdir "build\tools\innosetup" || goto error
mkdir "build\tools\innosetup" || goto error
"build\externals\innosetup-6.5.4.exe" /PORTABLE=1 /CURRENTUSER /DIR="%CD%\build\tools\innosetup\{app}" /LOG="%CD%\build\tools\innosetup\setup.log" /SILENT || goto error
