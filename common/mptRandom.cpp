...
sane_random_device::result_type sane_random_device::operator()()
{
	...
	if(prd)
	{
		try
		{
			if(prd->min() != 0 || !mpt::is_mask(prd->max()))
			{ // insane std::random_device
				//  This implementation is not exactly uniformly distributed but good enough
				// for OpenMPT.
				double rd_min = static_cast<double>(prd->min());
				double rd_max = static_cast<double>(prd->max());
			...
			{ // sane std::random_device
				result = 0;
				std::size_t rd_bits = mpt::lower_bound_entropy_bits(prd->max());
