...
#if MPT_COMPILER_GENERIC || (MPT_COMPILER_MSVC && MPT_MSVC_AT_LEAST(2010,0)) || (MPT_COMPILER_GCC && MPT_GCC_AT_LEAST(4,4,0)) || (MPT_COMPILER_CLANG && MPT_CLANG_AT_LEAST(3,0,0))
...
inline T random(Trng & rng)
{
	...
	const int rng_bits = mpt::engine_traits<Trng>::entropy_bits();
	unsigned_T result = 0;
