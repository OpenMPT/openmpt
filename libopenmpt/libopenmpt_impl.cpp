...
void log_interface::AddToLog( LogLevel level, const std::string & text ) const {
...
module_impl::loader_log::loader_log( module_impl & s ) : self(s) {
	return;
}
void module_impl::loader_log::AddToLog( LogLevel level, const std::string & text ) const {
	self.m_loaderMessages.push_back( std::make_pair( level, text ) );
}

void module_impl::PushToCSoundFileLog( LogLevel level, const std::string & text ) const {
...
void module_impl::apply_mixer_settings( std::int32_t samplerate, int channels, SampleFormat format ) {
	if (
...
module_impl::module_impl( std::istream & stream, std::shared_ptr<log_interface> log ) : m_LogForwarder(log) {
	init();
	load( FileReader( &stream ) );
	apply_libopenmpt_defaults();
}
module_impl::module_impl( const std::vector<std::uint8_t> & data, std::shared_ptr<log_interface> log ) : m_LogForwarder(log) {
	init();
	load( FileReader( data.data(), data.size() ) );
	apply_libopenmpt_defaults();
}
module_impl::module_impl( const std::vector<char> & data, std::shared_ptr<log_interface> log ) : m_LogForwarder(log) {
	init();
	load( FileReader( data.data(), data.size() ) );
	apply_libopenmpt_defaults();
}
module_impl::module_impl( const std::uint8_t * data, std::size_t size, std::shared_ptr<log_interface> log ) : m_LogForwarder(log) {
	init();
	load( FileReader( data, size ) );
	apply_libopenmpt_defaults();
}
module_impl::module_impl( const char * data, std::size_t size, std::shared_ptr<log_interface> log ) : m_LogForwarder(log) {
	init();
	load( FileReader( data, size ) );
	apply_libopenmpt_defaults();
}
module_impl::module_impl( const void * data, std::size_t size, std::shared_ptr<log_interface> log ) : m_LogForwarder(log) {
	init();
	load( FileReader( data, size ) );
	apply_libopenmpt_defaults();
}
