...
LIBOPENMPT_MODPLUG_API ModPlugFile* ModPlug_Load(const void* data, int size)
{
	...
	file->mod = openmpt_module_create_from_memory(data,size,NULL,NULL);
	if(!file->mod){
		free(file);
		return NULL;
	}
...
LIBOPENMPT_MODPLUG_API unsigned int ModPlug_GetMasterVolume(ModPlugFile* file)
{
	...
	if(!file) return 0;
	val = 0;
	if(!openmpt_module_get_render_param(file->mod,OPENMPT_MODULE_RENDER_MASTERGAIN_DB,&val)) return 128;
	return (unsigned int)(128.0*pow(10.0,val*0.05));
}

LIBOPENMPT_MODPLUG_API void ModPlug_SetMasterVolume(ModPlugFile* file,unsigned int cvol)
{
	if(!file) return;
	openmpt_module_set_render_param(file->mod,OPENMPT_MODULE_RENDER_MASTERGAIN_DB,(int32_t)(20.0*log10(cvol/128.0)));
}
