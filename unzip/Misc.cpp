...
void CZipArchive::UpdateCRC(BYTE *s, int len)
//-------------------------------------------
{
    DWORD crcval = crc32val;
...
int CZipArchive::DoString(unsigned int len, int option)
//-----------------------------------------------------
{
    int block_length, error = 0;
    WORD comment_bytes_left, extra_len;
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    case FILENAME:
        ...
        if (ReadBuf(filename, len) <= 0) return 51;
        filename[len] = '\0';   /* terminate w/zero:  ASCIIZ */

		// Since we unzip in memory, we don't care about the case
		// if (pInfo->lcflag)
		//	TOLOWER(filename, filename);  // replace with lowercase filename

        if (!extra_len) break;        // we're done here
    ...
    case SKIP:
        if (!Seek(cur_zipfile_bufstart + (inptr-inbuf) + len)) return 51;
        break;
    ...
    case EXTRA_FIELD:
        if (extra_field != NULL) delete extra_field;
        if ((extra_field = new BYTE[len]) == NULL) 
		{
            if (!Seek(cur_zipfile_bufstart + (inptr-inbuf) + len)) return 51;
        } else
            if (ReadBuf((char *)extra_field, len) <= 0)	return 51;
        break;
